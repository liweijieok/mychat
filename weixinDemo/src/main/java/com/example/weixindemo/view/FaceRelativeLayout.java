...
import com.example.weixindemo.adapter.FaceAdapter;
...
public class FaceRelativeLayout extends RelativeLayout implements OnItemClickListener, OnClickListener {
    ...
    public static final int CLOSE_MORE_OPERATE = AtomIntegerUtil.getDiffIntCode();//关闭聊天框下方的其他操作的界面
    public static final int READ_EMOJI = AtomIntegerUtil.getDiffIntCode();//异步读取表情
    ...
    private void onCreate() {
        handler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                if (msg.what == CLOSE_MORE_OPERATE) {
                    if (view.getVisibility() == View.VISIBLE) {
                        view.setVisibility(View.GONE);
                    }

                } else if (msg.what == READ_EMOJI) {
                    //完成初始化
    ...
    private void readEmoji() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                FaceConversionUtil.getInstace().getFileText(getContext());
                handler.sendEmptyMessage(READ_EMOJI);
            }
        }).start();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_face:
                //表情点击的时候的处理
                //假如当前已经显示了表情则，切换到打开输入法，
                //假如现在是输入法，则打开表情
                if (view.getVisibility() == View.GONE) {
                    InputMethodUtil.hideInput(getContext(), et_sendmessage);
                    view.setVisibility(View.VISIBLE);
                } else {
                    InputMethodUtil.showInput(getContext(),et_sendmessage);
                    view.setVisibility(View.GONE);
                }
                break;
            ...
            case R.id.ivPopUp:
                ...
                //handlerInput.sendEmptyMessage(CLOSE_INPUT);
                // 隐藏表情选择框
                if (view.getVisibility() == View.VISIBLE) {
                    view.setVisibility(View.GONE);
                }
